...
_DESCRIPTION = """\
SARI+BP modifies the original SARI metric by adding a
Brevity Penalty factor, similar to that used in BLEU.

The regular SARI metric, which is designed for evaluating
simplified sentences, doesn't penalize short system
outputs.

For open-ended conditional text generation (e.g. review
response generation), we may want to
penalize system outputs that contain good overlap with a
reference but fail in terms of length as these are likely to be overly
generic.

E.g.

sources=["we enjoyed the schnitzels and the staff are friendly"]
references=[["thank you for your review. we are very happy to hear that you enjoyed your meals and that our staff provided excellent service."]]
hyp1 = ["thank you for taking the time to write a review. we are happy to hear that you enjoyed the schnitzels."]
hyp2 = ["thank you for your review."]
...
For more information on the original metric, see _DESCRIPTION
in sari.py

DISCREPENCY: 
    Original implementation
    (https://github.com/cocoxu/simplification/blob/master/SARI.py)
    0/0 = 0 in case of 0 precision/recall
...
def get_closest_ref_len(hyp_len: int, ref_lens: List[int]) -> int:
    """
    From sacrebleu:
...
def SARIngram(sgrams, cgrams, rgramslist, numref, div_zero):
    ...
    addscore_precision = div_zero
    addscore_recall = div_zero
...
def SARIsent(ssent, csent, rsents, use_brevity_penalty, div_zero):
    numref = len(rsents)
    ...
    # adapted from https://github.com/mjpost/sacrebleu/blob/master/sacrebleu/metrics/bleu.py
    if use_brevity_penalty:
        rsent_lens = [len(rsent.split(" ")) for rsent in rsents]
        csent_len = len(c1grams)
        ssent_len = len(s1grams)
        
        if csent_len < max(rsent_lens):
            # include the source as a candidate for getting the
            # closest length reference.
            # closest_rsent_len = get_closest_ref_len(csent_len, rsent_lens + [ssent_len])
            closest_rsent_len = get_closest_ref_len(csent_len, rsent_lens)
            BP = math.exp(1 - closest_rsent_len / csent_len) if csent_len > 0 else 0.0
        else:
            BP = 1.0
...
class SariBP(datasets.Metric):
    ...
    def _compute(self, sources, predictions, references, use_brevity_penalty=True):
